  ...
  let disposableRenameCommand = vscode.commands.registerCommand(
    ...
    (clicked_file, selected_files) => {
      ...
      selected_files.forEach((file) => {
        ...
        file.basepath = file.fsPath.substring(
          0,
          file.fsPath.lastIndexOf(file.basename)
        );
        current_renaming.files.push(file);
      });

      let batchFilePath = PathModule.join(__dirname, ".Batch Rename.txt");
      let content = current_renaming.files
          ...
          if (current_renaming.files.length == new_names.length) {
            ...
            for (let i = 0; i < current_renaming.files.length; i++) {
              ...
              while (fs.existsSync(adjusted_new_path)) {
                adjusted_new_path = PathModule.join(
                  file.basepath,
                  new_name.replace(/\.(?=[A-z0-9]*$)/, `_${num}.`)
                );
                num++;
              }
